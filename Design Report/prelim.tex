...
The analog portion of the circuit is a single IC consisting of four operational amplifiers, three of which are used. See Figure \ref{analog_block} for overview, each stage is one opamp. The Pre-Amp is simply a voltage follower. The Filter is a simple inverting amplifier configuration with filtering caps. The Square Wave Generator is simply configured as a comparator. The output of which is then fed to the microcontroller.
...
Power consumption was a big issue for this project because it is  meant to be run off og battery power. We will be powering the circuit from a rechargeable lithium polymer battery. Due to stock and size constraints we are limited to two options for capacity, 150 mAh and 3 Ah. Even if we assume the worst case calculations, see Table \ref{Power}, the 150 mAh battery will power the circuit for almost 3 hours. The real time will be closer to seven hours as we will be spending most of our time with the microcontroller sleeping.
